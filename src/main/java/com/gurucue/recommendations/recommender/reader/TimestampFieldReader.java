 ...
 * contain a single timestamp value.
 * @see ProductReader
 */
public final class TimestampFieldReader implements FieldReader {
    private final Attribute attribute;
    private final int attributeIndex;

    public TimestampFieldReader(final Attribute attribute, final int attributeIndex) {
    ...
    public void process(final ProductData newProductData, final AttributeValues attributes) {
        if (attributes.contains(attribute)) {
            newProductData.setAttrValue(attributeIndex, Long.toString(attributes.getAsInteger(attribute) / 1000L, 10));
        }
    }
}
